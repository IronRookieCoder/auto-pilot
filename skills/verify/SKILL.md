...
```
[!] 验证失败时，只允许修改实现代码来通过测试。
[!] 禁止修改、删除、跳过或弱化任何测试用例。
[!] 禁止通过修改测试断言、放宽匹配条件、添加 skip/ignore 来"通过"验证。
[!] 如果测试本身确实有 bug，记录为"阻塞"，由用户决定。
```
...
| 步骤 | 状态 | 详情 |
|------|------|------|
| Lint | PASS/FAIL | {通过/N 个问题} |
| TypeCheck | PASS/FAIL | {通过/N 个错误} |
| Test | PASS/FAIL | {N 通过, N 失败, N 跳过} |
| Build | PASS/FAIL | {通过/失败原因} |
