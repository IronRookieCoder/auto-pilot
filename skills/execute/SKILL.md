...
```
execute（编排者）
  ├─ 读取 workflow.json.current_milestone_id / milestones.json
  ├─ 优先恢复当前 failed/in_progress 里程碑
  ├─ 仅在不存在当前失败/进行中里程碑时，定位下一个 pending 里程碑
...
2. 读取 `workflow.json`，检查：
   - `status` 字段：如果为 `blocked`/`failed`/`paused`，向用户展示 `reason`，等待用户决策；只有实际执行 `python tools/workflow_resume.py` 成功后才能继续
...
如果执行被中断，重新调用时：
1. 优先读取 `workflow.json.current_milestone_id`
2. 如果该里程碑存在且状态为 `failed` / `in_progress` / `pending`，从该里程碑继续
3. 如果 `current_milestone_id` 为空，但存在任意 `failed` 里程碑 → 停止并修正状态或调整计划；不得恢复
4. 如果没有当前里程碑，再读取 `milestones.json` 找到第一个 `in_progress`
5. 仍不存在时，再选择依赖已满足的下一个 `pending`
6. 不得因为存在 failed 里程碑就直接跳到下一个 `pending`

**边界情况处理**：
- 里程碑状态为 `in_progress` 但子代理中途中断 → 子代理不直接写状态文件，因此代码可能写了一半但状态未更新。重新为该里程碑生成子代理，子代理应检查已有的测试和实现文件，从当前实际进度继续
- `current_milestone_id` 指向的里程碑已为 `completed` → 说明状态更新不完整，执行 lint 检查，修正后再选择下一个 pending 里程碑
- `current_milestone_id` 指向的里程碑为 `failed` → 只能重试该里程碑，或等待用户调整计划/依赖关系；不得直接跳到下一个 pending
- 存在非当前 failed 里程碑 → 不得恢复到其他 pending / in_progress 里程碑；必须先处理失败项或调整计划
...
- **failed 里程碑不得直接越过**：未重试或未调整计划前，不得直接执行下一个 pending
