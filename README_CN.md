...
- **`PreToolUse`**（matcher: `Write|Edit`）：调用 [`hooks/validate_workflow_write.py`](hooks/validate_workflow_write.py)
...
- **`PostToolUse`**（matcher: `Skill`）：调用 [`hooks/post_skill_lint.py`](hooks/post_skill_lint.py)
  - 在每次技能执行结束后自动运行 `workflow_lint.py`
  - 若产物存在不一致，则阻断后续推进
